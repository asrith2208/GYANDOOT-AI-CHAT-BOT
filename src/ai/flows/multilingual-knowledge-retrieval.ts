...
export type MultilingualKnowledgeRetrievalInput = z.infer<typeof MultilingualKnowledgeRetrievalInputSchema>;

const MultilingualKnowledgeRetrievalOutputSchema = z.object({
  answer: z.string().describe('The answer to the user query, extracted from the university website links.'),
  language: z.string().describe('The language of the answer.'),
});
export type MultilingualKnowledgeRetrievalOutput = z.infer<typeof MultilingualKnowledgeRetrievalOutputSchema>;

export async function multilingualKnowledgeRetrieval(input: MultilingualKnowledgeRetrievalInput): Promise<MultilingualKnowledgeRetrievalOutput> {
  return multilingualKnowledgeRetrievalFlow(input);
}
...
Knowledge Base:
{{#each knowledgeBaseLinks}}- {{{this}}}
{{/each}}

Question: {{{query}}}

Answer:`, // Ensure output is in the same language as the input question.
});

const multilingualKnowledgeRetrievalFlow = ai.defineFlow(
  {
    name: 'multilingualKnowledgeRetrievalFlow',
    inputSchema: MultilingualKnowledgeRetrievalInputSchema,
    outputSchema: MultilingualKnowledgeRetrievalOutputSchema,
  },
  async (input) => {
